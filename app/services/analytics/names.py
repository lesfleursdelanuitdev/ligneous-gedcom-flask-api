"""Personal names: given-name and surname frequency, sex splits, decade and place slices.

Charts and graph data for `gedcom_given_names_v2` / `gedcom_surnames_v2` and
their joins to individuals and places.
"""
...
def get_given_names_statistics(tree_id: str, limit: int) -> dict | None:
    """Aggregates and distributions for `gedcom_given_names_v2` (per tree)."""
...
def get_surnames_statistics(tree_id: str, limit: int) -> dict | None:
    """Aggregates and distributions for `gedcom_surnames_v2` (per tree)."""
    file_uuid = get_file_uuid_for_tree(tree_id)
    if not file_uuid:
        return None
    with get_connection() as conn:
